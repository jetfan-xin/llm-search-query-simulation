...
class QuerySimulatorOptimized:
    ...
    def load_initial_data(self):
        with open(os.path.join('data', 'data_update0302.json'), 'r', encoding='utf-8') as file:
    ...
    def load_corpus(self):
        ...
        for corpus_name in ['task_description', 'title', 'abstract', 'content']:
            with open(os.path.join('baseline', '2', 'corpus', f'{corpus_name}.json'), 'r', encoding='utf-8') as f:
        ...
        if self.len_strategy == 'token_length':
            for q_length in random_query_length:
                ...
                while surplus_length > 0:
                    new_term = random.sample(corpus, 1)
                    try_times = 10
    ...
    def evaluation_bleu_with_tasks(self):
        ...
        for task_id in self.queries:
            ...
            filename = os.path.join('baseline', '2', 'output', f'output_task_{self.len_strategy}_{task_id}.csv')
    ...
    def evaluation_bleu_with_users(self):
        ...
        for user_id, user_queries in self.queries_with_users.items():
            ...
            filename = os.path.join('baseline', '2', 'output',  f'output_user_{self.len_strategy}_{self.term_strategy}_{user_id}.csv')
            with open(filename, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerows(evaluation_all)
    ...
    def ranked_for_users(self):
        ...
        file_pattern = os.path.join('baseline', '2', 'output', f'output_user_{self.len_strategy}_{self.term_strategy}_*.csv')
        ...
        for filename in csv_files:
            ...
            with open(
                    os.path.join('baseline', '2', 'output', f'output_user_{self.len_strategy}_{self.term_strategy}_{user_id}_ranked.csv'),
                    'w', newline='') as file:
                writer = csv.writer(file)
    ...
    def caculate_best_args(self):
        ...
        file_pattern = os.path.join('baseline', '2', 'output', f'output_task_{self.len_strategy}_*.csv')
        csv_files = glob.glob(file_pattern)
        ...
        for file in csv_files:
            ...
            if not selected_rows.empty:
                ...
                task_id = selected_rows.iloc[0]['task_id']
        ...
        output_df = output_df.append(overall_avg_row, ignore_index=True)

        file_pattern = os.path.join('baseline', '2', 'output', f'output_user_{self.len_strategy}_{self.term_strategy}_*_ranked.csv')
        csv_files = glob.glob(file_pattern)
        ...
        output_df = output_df.append(users_avg_row, ignore_index=True)

        output_file = os.path.join('baseline', '2', 'output', "baseline1_best_args.csv")
    ...
    def simulation_with_best_args(self, alpha, beta, gamma):
        ...
        filename = os.path.join('baseline', '2', 'output', f'baseline2_data.json')
        with open(filename, 'w', encoding='utf-8') as file:
            json.dump(output_data, file, default=lambda o: o.__dict__, indent=4, ensure_ascii=False)

if __name__ == '__main__':

    q = QuerySimulatorOptimized('term_length', 'random')

    q.simulation_with_best_args(100, 0, 0)
